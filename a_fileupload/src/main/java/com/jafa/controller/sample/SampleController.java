...
public class SampleController {
	...
	public String fileUpload(String title, @RequestParam("attachFile") MultipartFile multipartFile) {
		log.info(title);
		...
		log.info(multipartFile.getName()); // input 태그의 name 속성
		log.info(multipartFile.getContentType()); // 파일타입
	...
	// 요청 /download?fileName=뱅갈호랑이.png
	// 파일 다운로드
	@GetMapping(value = "/sample/download", produces = MediaType.APPLICATION_OCTET_STREAM_VALUE)
	@ResponseBody
	public ResponseEntity<Resource> download(@RequestHeader("user-Agent") String userAgent, String fileName) throws UnsupportedEncodingException{
		log.info(userAgent);
	...
	// 삭제
	// 요청 : /sample/delete?fileName=뱅갈호랑이.png
	...
	// /sample/imgDisplay?fileName=귤.png
	// 이미지 파일 표시
