...
public class BoardDao {
	...
	class RowMapperIpl implements RowMapper<BoardVO>{
		@Override
	...
	public BoardVO getByBno(Long bno) {
		BoardVO vo = null;
		String sql = "select * from board_t where bno=?";
		try {
			vo = jdbcTemplate.queryForObject(sql, boardMapper, bno);
			
		} catch (EmptyResultDataAccessException e) {
			System.out.println("찾는 게시물 없음");
		}
	...
	public Long insertKeyHolder(BoardVO vo) {
		KeyHolder keyHoder = new GeneratedKeyHolder(); // 처음으로 생성된 키값을 구하는 객체
		...
		Long key = keyHoder.getKey().longValue();
		vo.setBno(key);
		return key;
