...
// @RequestMapping, @GetMapping, @PostMapping, 커맨드 객체
@Controller
@RequestMapping("/sample02") // 클래스단위에서 @RequestMapping 적용
...
public class SampleController02 {
	...
	public String list4(@ModelAttribute("svo") SampleVO vo) {
		// 컨트롤러 메소드에 사용된 참조타입을 커맨드 객체라함(String, Wrapper클래스 제외)
		// 커맨드 객체는 Model에 데이터를 등록하지 않고 뷰에서 사용할 수 있다.
		// 이때 뷰페이지에서 사용할 이름은 sampleVO이다. (타입에서 첫글자를 소문자로 변경할 이름)
		// 뷰에서 사용할 이름을 변경하려면 @ModelAttribute 사용
		// 커맨드 객체를 샤용하려면 기본생성자가 반드시 있어야함
