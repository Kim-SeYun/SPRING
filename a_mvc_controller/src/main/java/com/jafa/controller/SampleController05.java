...
public class SampleController05 {
	...
	// 요청 파라미터 값을 커맨드 객체의 프로퍼티에 저장할 경우 세터 매소드 이용
	
	@RequestMapping(value = "/register", method = RequestMethod.POST)
	public String register(MemberDTO memberDTO) { // 커맨트 객체
		// 커맨드 객체 : 참조타입(기본형과 기본형의 래퍼타입 제외)이 파라미터로 선언
		// Model에 추가하지 않고 뷰 페이지에서 사용할 수 있다.
		// 뷰 페이지에서 사용되는 이름은 memberDTO(타입의 첫글자 소문자 변경)
